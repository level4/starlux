...
pub fn evaluate_starlark_code(code: &str) -> Result<(String, String), starlark::Error> {
        ...
        let extensions = [Json, Print, RecordType, StructType, Pprint, Typing, EnumType,Map, Filter, Partial, Debug];
    let globals = GlobalsBuilder::extended_by(&extensions).with(starlark_emit).build();
    let module = Module::new();
    let store = Store::default();
